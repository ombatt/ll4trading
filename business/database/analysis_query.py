    ...
    for h_data in hist_data:
        ...
        for doc in last_ana_documents:
            ...
            if h_data.date.strftime('%Y.%m.%d') == data_formattata:  # and 'close' not in doc
                ...
                if float(close) >= 1.5:
                    hesit = 2
                elif 0.5 <= float(close) < 1.5:
                    hesit = 1
                elif -0.5 < float(close) < 0.5:
                    hesit = 0
                elif -1.5 < float(close) <= -0.5:
                    hesit = -1
                if float(close) <= -1.5:
                    hesit = -2'''
...
def read_last_analysis() -> [Analysis]:
    ...
    try:
        ...
        _1_days = datetime.now(tz.UTC) - timedelta(days=1)
        recent_an = news_database.search(Query().date.test(lambda date_str: parse(date_str) > _1_days))
